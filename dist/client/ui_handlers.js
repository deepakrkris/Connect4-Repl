...
join_btn.addEventListener('click', function (event) {
    const userId_e = document.getElementById('userId');
    const gameCode_e = document.getElementById('gameCode');
...
function handleUserClickAction(target) {
    const row_clicked = parseInt(target.attributes.getNamedItem('row').value);
    const side = target.attributes.getNamedItem('side').value;
    const client_state = ClientConnection.client_state;
    const message = {
        row: row_clicked,
...
export function handleUserActionNotification(data) {
...
export function handleGameSessionNotification(data) {
    const client_state = ClientConnection.client_state;
...
export function handleGameNotification(data) {
